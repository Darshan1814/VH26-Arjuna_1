    ...
    <div className="w-full flex-1 py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-4">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.back()}
            className="rounded-lg border p-2 hover:bg-[var(--color-surface)] text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition"
          >
      ...
      {loading ? (
        <div className="flex h-96 items-center justify-center">
          <div className="flex flex-col items-center gap-2 text-xs text-[var(--color-text-muted)]">
            ...
            <span>Loading diagnostic report #{reportId}...</span>
          </div>
        </div>
      ) : error ? (
        <div className="rounded-xl border border-red-300 bg-red-50 dark:bg-red-950/20 p-6 text-center space-y-2">
          ...
          <p className="text-sm font-semibold text-red-700 dark:text-red-400">Report Unavailable</p>
