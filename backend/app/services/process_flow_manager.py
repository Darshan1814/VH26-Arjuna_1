...
class ProcessFlowManager:
    ...
    async def execute_step(
        ...
        if step_num == 1:
            ...
            # Use OpenAI to create a technical profile of the uploaded assets
            ai_profile = {}
  ...
  "readiness_verdict": "Verified & Ready for Diagnostic Indexing"
        ...
        # ---------------------------------------------------------------------
            ...
            result = {
                ...
                "pages_processed": total_pages if total_pages > 0 else (1 if session.files else 0),
...
{{
  ...
  "mandatory_safety_precautions": ["Precaution 1", "Precaution 2", "Precaution 3"]
        ...
        # ---------------------------------------------------------------------
            ...
            # If still empty (e.g. text file or unparsed format), split raw text into clean chunks dynamically
            if not session.chunks and session.normalized_docs:
                ...
                machine_label = session.selected_machine or "Universal Equipment"
                session.chunks = [
                    {
                        "chunk_index": idx,
            ...
            for c in session.chunks[:6]:
                chunk_previews.append({
                    ...
                    "machine": c.get("machine_model", session.selected_machine or "Universal Equipment"),
                })
        ...
        # ---------------------------------------------------------------------
            ...
            if not raw_query or len(raw_query.strip()) < 3:
                raw_query = f"What are the primary troubleshooting steps and fault recovery procedures for {clean_equip}?"
            ...
            machine = analysis.get("machine_model") or clean_equip
            detected_errs = analysis.get("error_codes") or []
            err_code = detected_errs[0] if detected_errs else "OPERATIONAL_FAULT"
            ...
            if session.chunks:
                ...
                retrieved = [
                    RetrievedChunk(
                        ...
                        match_type="keyword" if m_score > 0 else "vector",
                    )
